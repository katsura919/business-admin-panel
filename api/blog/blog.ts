...
export const getAllBlogs = async (params?: { businessId?: string; status?: string }): Promise<Blog[]> => {
...
export const getBlogsByBusiness = async (businessId: string): Promise<Blog[]> => {
    const response = await api.get<Blog[]>(`/businesses/${businessId}/blogs`);
    return response.data;
};
...
// Update blog
export const updateBlog = async (id: string, data: UpdateBlogRequest): Promise<Blog> => {
